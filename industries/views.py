...
def getAllIndustries(request):
    ...
    sheet = client.open("TestSheet47").worksheet('Industries')

    data = sheet.get_all_records()

    return render(request, 'industries.html', {'data' : data})
...
def getIndustry(request):
    ...
    for obj in data:
        if obj["Industries"][0] == val1:
            specfic.append(obj)

    return render(request, 'industries.html', {'data' : specfic})
